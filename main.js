...
function edit(id){
	...
	var todo_item=storedNames[clickedtodoIndex];
        ...
        var date=$('.date').val(todo_item.start);
...
localStorage.setItem("array", JSON.stringify(array));
        ...
        $("#todo-form")[0].reset();
         ...
         var wrapper = '<div class="task-header">'+title+'</div><div class="task-date">'+date+'</div><div class="task-description">'+des+'</div><div class="task-tags">'+tag+'</div><div onclick="edit(this.id)" id="'+id+'" class="btn btn-primary edit">Edit</div><div onclick="del(this.id)" id="'+id+'" class="btn btn-danger del">Delete</div>'
       parent.html(wrapper);
